...
export const AddApartmentToFavorites = observer(({ apartmentId, variant = "medium" }: PropsType) => {
   const { t }:{t: typeof i18next.t} = useTranslation();
   ...
   const toggleInFavoriteList = () => {
      ...
      if (favoritesStore.favorites.includes(apartmentId)) {
         snackBarStore.showSnackBar("Квартира добавлена в избранное", {
            timeout: 1100,
         });
      }
   };
   ...
   if (variant === "mini") {
      return <SvgButton className={clsx("add-to-wish-list-button", "mini", className)} icon={buttonIcon} asImage={false} onClick={toggleInFavoriteList} />;
   }

   if (favoritesStore.favorites.includes(apartmentId)) {
      return <Button className={clsx("add-to-wish-list-button", className)} icon={buttonIcon} onClick={toggleInFavoriteList} title={buttonTitle} />;
   } else {
      return <Button className="add-to-wish-list-button" icon={buttonIcon} onClick={toggleInFavoriteList} title={buttonTitle} />;
   }
});
