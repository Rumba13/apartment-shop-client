...
export const SelectGuestsCountInputNumber = observer(({}: PropsType) => {
   ...
   return (
      <div className="select-guests-count-input-number">
         ...
         <input className="input-number__value" value={inputValue} ref={inputRef} onChange={onChange} />

         <button className="input-number-increase" onClick={() => guestStore.setGuestsCount(guestStore.guestsCount + 1)}>
            <SvgIcon icon={Arrow} />
         </button>
         <button className="input-number-decrease" onClick={() => guestStore.setGuestsCount(guestStore.guestsCount - 1)}>
            <SvgIcon icon={Arrow} />
         </button>
      </div>
   );
