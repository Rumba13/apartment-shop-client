...
export const OrderCard = observer(({ order }: PropsType) => {
   ...
   return (
      <div className="order">
         <div className="created-at">{dayjs(order.fromDate).locale("ru").format("DD.MM.YYYY hh:mm")}</div>

         <div className="order-main">
            <Link className="order-photo-wrapper" to={`/apartment-details/${apartmentFromOrder.id}`}>
               <img className="order-photo" src={apartmentFromOrder.photos[0] ? CONSTANTS.IMAGE_SERVER_URL + apartmentFromOrder.photos[0] : NoImage} alt="" />
            </Link>
            <Link className="order__title" to={`/apartment-details/${apartmentFromOrder.id}`}>
               {t("Long Term Rental")}
            </Link>
