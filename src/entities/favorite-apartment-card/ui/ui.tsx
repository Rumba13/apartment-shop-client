...
export const FavoriteApartmentCard = observer(({apartmentId}: PropsType) => {
    ...
    if (!apartment) return <div className="favorite-apartment-card">
        <Skeleton/>
    </div>

    return <div className="favorite-apartment-card">
        <AddApartmentToFavorites apartmentId={apartmentId} variant="mini"/>
        <Link className="apartment-title" to={`/apartment-details/${apartmentId}`}>{apartment.title}</Link>
        {apartment.address}
    </div>
})
