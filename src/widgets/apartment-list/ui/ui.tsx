...
export const ApartmentList = observer(({}: PropsType) => {
   ...
   if (apartmentListStore.apartments === null) {
      //initial loading
      return (
   ...
   return (
      <div className="apartment-list">
         <div className={clsx("apartment-list-loading", apartmentListStore.isLoading && "opened")}></div>
         <MapModal />

         {apartmentListStore.apartments.length === 0 ? t("Nothing Found") : apartmentListStore.apartments.map(apartment => <ApartmentCard apartment={apartment} key={apartment.id} />)}
      </div>
   );
});
