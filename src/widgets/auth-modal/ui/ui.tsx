...
export const AuthModal = observer(() => {
   ...
   return (
      <Modal className={clsx("auth-modal")} title="" modalStore={authModalStore}>
         <div className="auth-modal-tabs">
            {[t("Sign In"), t("Sign Up")].map((tabTitle, index) => (
               <button key={tabTitle} className={clsx("auth-modal-tab", index === authModalStore.activeTab && "active")} onClick={() => authModalStore.setActiveTab(index)}>
                  {tabTitle}
               </button>
            ))}
         </div>
