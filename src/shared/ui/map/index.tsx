...
export function Map({ address }: PropsType) {
   ...
   useEffect(() => {
      ymapsApi?.geocode(address).then(res => {
         //@ts-ignore
   ...
   return (
      <div className="map">
         <YMaps query={{ apikey: "20c277cb-5d76-4dbd-9f47-66fd833f5afc" }}>
            <YMAP state={{ center: coordinates || [40, 40], zoom: 14 }} options={{ suppressMapOpenBlock: true }} style={{ width: "100%", aspectRatio: 16 / 9 }} modules={["geolocation", "geocode"]} onLoad={ymaps => setYmapsApi(ymaps)}>
               {coordinates && <Placemark geometry={coordinates} properties={{ iconCaption: address }} />}
